(function () {
  ...
  async function loadCharacters() {
    try {
      const res = await fetch("characters_en.json", { cache: "no-cache" });
      if (!res.ok) throw new Error("failed to load characters_en.json");
  ...
  function openBestScoresWindow() {
    ...
    if (btnClear) {
      ...
      const runClear = (e) => {
        e.preventDefault();
        if (confirm('Delete all best scores?')) {
          clearBestScores();
        }
      };
  ...
  function levelDescription(level) {
    ...
    if (!ch) {
      if (lvl === 1) return "Lv1: Beginner";
      if (lvl === 2) return "Lv2: Intermediate";
      if (lvl === 3) return "Lv3: Advanced";
      if (lvl === 4) return "Lv4: Master";
      return "Lv5:CPU";
    }
  ...
  function roleFor(player) {
    if (vsCpu) {
      if (player === humanColor) return "(You)";
      return levelName(cpuLevel);
    }
    if (networked) {
      if (player === myColor) return "(You)";
      return "(Challenger)";
    }
    return "";
  }
  ...
  function updatePlayerLabels() {
    if (labelBlackEl) labelBlackEl.textContent = `BLACK${roleFor(BLACK)}`;
    if (labelWhiteEl) labelWhiteEl.textContent = `WHITE${roleFor(WHITE)}`;
    
    let modeText = "vs AI";
    if (vsCpu) {
      const ch = getCharacter(cpuLevel);
      modeText = `vs AI (${ch?.name || ""})`;
    } else if (networked) {
      modeText = "vs Online";
    }
    if (subtitleEl) subtitleEl.textContent = `${modeText} — Black goes first.`;
  ...
  function turnLabel() {
    if (gameOver) return "Game set";
    if (vsCpu) {
      if (currentPlayer === humanColor) return "Your turn";
      return "Waiting... ";
    }
    if (networked) {
      return currentPlayer === myColor ? "Your turn" : "Waiting... ";
    }
    return "";
  }
  ...
  function updateCharacterSpeech(type, context = {}) {
    ...
    if (charAvatarEl && charAvatarEl.tagName === "IMG") {
      if (ch.avatarUrl) {
        ...
        charAvatarEl.alt = ch.name || "Character";
      }
    }
  ...
  function passMessage(passedPlayer) {
    if (!vsCpu) {
      return `${colorName(passedPlayer)} passes. ${colorName(currentPlayer)} to move.`;
    }
    if (passedPlayer === cpuColor) {
      const ch = getCharacter(cpuLevel);
      const name = ch?.name || "CPU";
      return `${name} passes. Your turn.`;
    }
    return "You pass. CPU's turn.";
  }

  function updateHintButtonText() {
    const btnHint = document.getElementById("btn-hint");
    if (btnHint) {
      btnHint.textContent = hintVisible ? "Hide Hints" : "Show Hints";
    }
  }
  ...
  function resultMessage(black, white) {
    if (!vsCpu) {
      ...
      if (white > black) return `White wins! (${black} vs ${white})`;
    ...
    if (humanCount > cpuCount) {
      return `You win! (${humanCount} vs ${cpuCount})`;
    }
    if (humanCount < cpuCount) {
      const ch = getCharacter(cpuLevel);
      const name = ch?.name || "CPU";
      return `${name} wins! (${humanCount} vs ${cpuCount})`;
    }
    return `Draw! (${humanCount} vs ${cpuCount})`;
  }
  ...
  function finishGame() {
    ...
    const headline = black > white ? "Black wins!" : white > black ? "White wins!" : "It's a draw!";
    ...
    if (turnTextEl) turnTextEl.textContent = "Game set";
  ...
  function clearOnlineChoiceState(statusEl) {
    ...
    if (statusEl) {
      statusEl.textContent = 'The same color has been selected. Please choose again.';
    }
  }
  ...
  function startOnlineGame(passphrase, hintInit, speechEnabledParam) {
    ...
    if (!setupSupabaseClient()) {
      alert('Connection error: Your ad blocker or privacy settings may be blocking online play. Please disable your extensions or play against the CPU.');
      showSetup();
      return;
    }
    ...
    if (messageEl) messageEl.textContent = 'Waiting for room code... ' + room;
    ...
    try {
      ...
      if (statusEl) {
        statusEl.textContent = 'Waiting for a player to join with your room code...';
      }
    } catch (error) {
      console.error('Connection Error', error);
      if (statusEl) {
        statusEl.textContent = 'Failed to connect online. Your connection may be blocked.';
      }
    }
  ...
  function sendOnlineColorChoice(color, hintInit, speechEnabledParam, statusEl, modal) {
    ...
    if (statusEl) statusEl.textContent = `You have selected ${color === 'black' ? 'Black' : 'White'}. Waiting for your opponent...`;
  ...
  function resolveOnlineColors(hintInit, speechEnabledParam, statusEl, modal) {
    ...
    if (statusEl) statusEl.textContent = 'Press "Start" to start the game.';
    ...
    if (btnGameStart) {
      ...
      btnGameStart.onclick = () => {
        ...
        if (messageEl) messageEl.textContent = 'Game start! You are ' + (assigned === 'black' ? 'Black (moves first)' : 'White (moves second)');
      };
    }
  }
  ...
  function updateSetupLabels() {
    ...
    if (isCpu) {
      ...
      if (colorLegendEl) colorLegendEl.textContent = "Your color";
      if (colorHintEl) colorHintEl.textContent = "You play against the CPU. Black moves first.";
    } else {
